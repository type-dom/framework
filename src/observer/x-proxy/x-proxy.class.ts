import { deepClone, isArray, isBoolean, isNumber, isObject, isPrimitive, isString, isUndefined } from '@type-dom/utils';
...
export class XProxy<T extends IJsonData> implements IXProxy<T> {
  ...
  private defineProperty<K extends keyof T>(key: K, initialValue?: T[K]): void {
    Object.defineProperty(this, key, {
      ...
      set(newValue: T[K]) {
        console.log(`拦截到了对属性 "${String(key)}" 的赋值操作，新值为：`, newValue);
        ...
        // 调用 handler 的 set 方法（如果已实现）
          ...
          if (newValue instanceof XProxy) {
            ...
            this[key] = newValue;
          } else { // 新值不是 value 的情况， 要判断obj[key]本身是不是基础XProxy
            if (key === 'value') {
              if (this[key] === newValue) {
                return true;
              }
  ...
  toJsonData(): IJsonData {
    if (this._target === void 0) { // 检查 this._target 是否为 null 或 undefined
...
export function createProxy(target: IJsonDataProp): XProxy<IJsonData> {
  ...
  for(const [key, value] of Object.entries(target as IJsonData | IJsonData[])) {
...
export function makePropertyNonEnumerable<T extends object, K extends keyof T>(obj: T, key: K): void {
