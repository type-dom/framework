...
import type { ISlotItem, IChild, ITypeNode } from './core/abstracts/type-node/type-node.interface';
import type { ITextNode } from './dom/components/text-node/text-node.interface';
import type { ITypeElement } from './core/abstracts/type-element/type-element.interface';
...
export function arraySlot<T extends IChild = IChild>(slot?: ISlotItem<T> | void): T[] {
  if (slot === undefined) {
    return [];
  }
  if (isArray(slot)) {
