...
import { isRef, isSignal, isComputed, unref, MaybeRef, Ref, } from '../../../reactivity/ref';
...
export function setAttrObj<T extends Attributes>(el?: TypeNode, attrObj?: T): void {
  if (!el) return;
  if (attrObj) {
    // if (el instanceof TypeFragment) { // 不会自动下传，需要下级手动接收。
    //   onBeforeMount(() => {
    ...
    for (const key in attrObj) {
      if (Object.hasOwnProperty.call(attrObj, key)) {
        // todo 如何优化
        const value = (attrObj as any)[key] as MaybeRef<string | number>;
...
export function addAttrObj<E extends TypeNode>(el?: E, attrObj?: E['props']['attrObj']): void {
  // console.warn('addAttrObj . attrObj is ', attrObj);
  if (!el) return;
  if (attrObj) {
    // if (el instanceof TypeFragment) { // todo enrror 注释了，会报 Uncaught ReferenceError: Cannot access 'TypeElement' before initialization
    // }
    if (isRef(attrObj)) {
      effect(() => {
        resetAttrObj(el, unref(attrObj) as E['attrObj']);
      });
    } else {
      for (const key in attrObj) {
        if (Object.hasOwnProperty.call(attrObj, key)) {
 ...
 * @param attrObj
 */
export function resetAttrObj<T extends Attributes>(el: TypeNode, attrObj?: T): void {
  if (!el) return;
...
export function renderAttrProp(el: TypeNode | undefined, key: string, value?: MaybeRef<IPrimitive> | ClassValue): void {
  ...
  if (dom) {
    ...
    if (isRef(value)) {
      ...
      effect(() => {
        ...
        if (dom instanceof Element) {
          if (dom instanceof SVGElement) {
            patchProp(dom, key, dom?.getAttribute(key), raw, 'svg');
          } else {
            patchProp(dom, key, dom?.getAttribute(key), raw);
          }
      ...
      //     //   //   nothing;
      //     // } else {
      //     //   // key is type , raw is textarea
      //     //   // (dom as any)[key] = raw; // SVGElement 中有问题；TdInput textarea 中有问题；
      //     // }
      //     if (key === 'value') { // select 标签设置 value 值
      //       dom.value = raw;
      //     } else {
      //       dom.setAttribute(key, raw);
      //     }
...
export function addAttrClass(el: TypeNode, classValue: ClassValue): void {
  if (!el) return;
  // 要先判断className是否已经存在 todo el.props.class need how to do
  if (!el.attrObj) {
