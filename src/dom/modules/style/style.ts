...
export function renderRawStyle(
  ...
  if (!dom) {
    createDom(el); // 保证dom存在
    dom = el.dom as HTMLElement | SVGElement;
  }
...
export function addWidth(el: TypeHtml | TypeSvg | XElement, width: string | number): void {
  addStyleObj(el, { width: addUnit(width) });
}

export function setWidth(el: TypeHtml | TypeSvg | XElement, width: string | number): void {
  setStyleObj(el, { width: addUnit(width) });
}

export function addHeight(el: TypeHtml | TypeSvg | XElement, height: string | number): void {
  addStyleObj(el, { height: addUnit(height) });
}

export function setHeight(el: TypeHtml | TypeSvg | XElement, height: string | number): void {
  setStyleObj(el, { height: addUnit(height) });
}

export function addBackgroundColor(
  el: TypeHtml | TypeSvg | XElement,
...
export function removeStyleObj(el: TypeHtml | TypeSvg | XElement, styleObj: CSSProperties): void {
  if (!el) return;
 ...
 * @param mode
 */
export function show(el: TypeHtml | TypeSvg | XElement, mode?: Property.Display): void {
  if (!el) return;
  setStyleProp(el, 'display', mode ?? 'block'); // flex block inline-block
}

export function hide(el: TypeHtml | TypeSvg | XElement): void {
  if (!el) return;
