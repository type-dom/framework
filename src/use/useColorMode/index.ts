...
export type UseColorModeReturn<T extends string = BasicColorMode> =
  Signal<T | BasicColorSchema> & {
    ...
    system: Computed<BasicColorMode>
    state: Computed<T | BasicColorMode>
  }
...
export function useColorMode<T extends string = BasicColorMode>(
  ...
  const system = computed(() => preferredDark.get() ? 'dark' : 'light')

  const store = storageRef || (
    storageKey == null
      ? toRef(initialValue) as Signal<T | BasicColorSchema>
  ...
  const updateHTMLAttrs = getSSRHandler(
    ...
    (selector, attribute, value) => {
      const el = typeof selector === 'string'
        ...
        : unrefElement(selector) as Element
      if (!el) {
        return
      }
