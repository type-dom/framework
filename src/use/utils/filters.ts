...
export interface DebounceFilterOptions {
   ...
   * In milliseconds.
   */
  maxWait?: number; // MaybeRefOrGetter<number>
...
export function debounceFilter(ms: number, options: DebounceFilterOptions = {}) {
  ...
  const filter: EventFilter = (invoke) => {
    const duration = toValue(ms) as number;
    const maxDuration = toValue(options.maxWait) as number;
...
export function pausableFilter(extendFilter: EventFilter = bypassFilter): Pausable & { eventFilter: EventFilter } {
  ...
  return { isActive: isActive, pause, resume, eventFilter }
}
