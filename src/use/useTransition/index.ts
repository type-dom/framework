 ...
 */
export const TransitionPresets = /* #__PURE__ */ Object.assign({}, { linear }, _TransitionPresets) as Record<keyof typeof _TransitionPresets, CubicBezierPoints> & {
...
export function executeTransition<T extends number | number[]>(
  ...
  // console.warn('startedAt is ', startedAt);
  const endAt = Date.now() + duration;
  // console.warn('endAt is ', endAt);
  const trans = typeof options.transition === 'function'
    ? options.transition
    : (options.transition ?? linear);
  // console.warn('trans is ', trans);
...
// export function useTransition<T extends number[]>(source: [...T], options?: UseTransitionOptions): { [K in keyof T]: number }
...
export function useTransition(
  ...
  const outputRef = signal(sourceVal());

  // console.warn('source to ');
  ...
  watch(sourceVal, async (to) => {
      ...
      const toVal = (Array.isArray(to) ? to.map(toValue<number>) : toValue(to)) as number | number[];
      // console.warn('toVal is ', toVal);
      options.onStarted?.();

      await executeTransition(outputRef, outputRef.get(), toVal, {
        ...options,
