...
/**
 * 挂载元素
 * mountFragment 会覆写这个方法
 * @param element 除了 TypeFragment 的元素
 * @param container
 */
export function mountElement(element: TypeElement, container: RendererElement) {
  ...
  element.params.created?.bind(element)();
  element.created();
  ...
  setCurrentInstance(element);
  element.params.beforeMount?.bind(element)();
  element.beforeMount();
  ...
  if (container && element.dom) {
    ...
    if (hasVIf(element)) {
      ...
      if (condition) {
        ...
        // } else {
          // insertAnchor(element, container);
      ...
      // } else {
        if (isDescendant(container, element.dom)) {
          ...
          if (element.anchor && isDescendant(container, element.anchor)) {
            // todo no vIf should not have anchor in element;
  ...
  // }
  setCurrentInstance(element); // todo watch 优化 props.vIf的监听
  element.params.mounted?.bind(element)();
  element.mounted();
