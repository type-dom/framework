...
// type ComponentVNode = TypeNode & {
//   type: TypeNode // ConcreteComponent
// }
...
export function pushWarningContext(vnode: TypeNode): void {
  stack.push(vnode)
}
...
export function warn(msg: string, ...args: any[]): void {
  ...
  if (appWarnHandler) {
    ...
    const warnArgs = [`[TD warn]: ${msg}`, ...args]
    if (
      trace.length // &&
...
export function getComponentTrace(): ComponentTraceStack {
  let currentVNode: TypeNode | undefined = stack[stack.length - 1]
  if (!currentVNode) {
    return []
  }
  ...
  while (currentVNode) {
    ...
    const parentInstance: TypeNode | undefined = currentVNode.parent
...
function formatTraceEntry({ vnode, recurseCount }: TraceEntry): any[] {
  ...
  const isRoot = vnode ? vnode.parent == null : false
  ...
  const close = `>` + postfix
  return vnode.props
    ? [open, ...formatProps(vnode.props as Data), close]
    : [open + close]
}
