import { // EMPTY_OBJ,
  PatchFlags } from '@type-dom/utils';
...
import { ElementNamespace, RendererElement, RendererNode } from './renderer';
...
const hostCreateElement = nodeOps.createElement;
...
export const processFragment = (
  ...
  const fragmentStartAnchor = (n2.dom = n1 ? n1.dom : hostCreateText(''))!
  ...
  if (n1 == null) {
    hostInsert(fragmentStartAnchor, container as Element, anchor as Node)
    hostInsert(fragmentEndAnchor, container as Element, anchor as Node)
...
export const mountElement = (
  ...
  const { props, // shapeFlag, transition, dirs
  } = vnode

  const el = vnode.dom = hostCreateElement(
  ...
  if (vnode.props.nodeValue) {
    hostSetElementText(el as Element, vnode.props.nodeValue as string)
  } else if (vnode.childNodes) {
    mountChildren(
      vnode.childNodes,
      el,
      undefined,
  ...
  // }
  // scopeId
  setScopeId(el, vnode, vnode.scopedId, slotScopeIds,) // parentComponent)
  // props
  // if (props) {
...
const setScopeId = (
  ...
  if (scopeId) {
    hostSetScopeId(el as Element, scopeId)
  }
  if (slotScopeIds) {
    for (let i = 0; i < slotScopeIds.length; i++) {
      hostSetScopeId(el as Element, slotScopeIds[i])
    }
  }
  // if (parentComponent) {
...
type MountChildrenFn = (
  children: (TypeNode | string)[] | undefined,
  container: RendererElement,
  anchor: Comment | Text | undefined,
...
function resolveChildrenNamespace(
  ...
  return (currentNamespace === 'svg' && props.tag === 'foreignObject') ||
