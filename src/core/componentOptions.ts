...
import { isRef, Ref, WatchCallback, WatchOptions } from '../reactivity';
...
import { OptionMergeFunction } from '../dom/components/app/app.interface';
// import { isReservedPrefix } from './componentPublicInstance';
// import { traverse } from '../reactivity/watch';
// import { UnwrapMixinsType } from './componentPublicInstance';
 ...
 * ```
 */
export type ComponentCustomOptions = object
...
> extends LegacyOptions<Props, D, C, M, Extends, I, II, Provide>,
...
export type ComponentProvideOptions = ObjectProvideOptions | AnyFn
...
export function applyOptions(instance: TypeNode): void {
  ...
  for (const key in injectOptions) {
    ...
    if (isRef(injected)) {
      ...
      Object.defineProperty(ctx, key, {
        enumerable: true,
        configurable: true,
        get: () => (injected as Ref).get(),
        set: v => ((injected as Ref).set(v)),
      })
    } else {
      ctx[key] = injected
    }
...
export function resolveMergedOptions(
  ...
  const resolved: MergedComponentOptions = {}
...
export const internalOptionMergeStrats: Record<string, AnyFn> = {
  data: mergeDataFn,
...
function mergeDataFn(to: any, from: any) {
  ...
  return function mergedDataFn(this: TypeNode) {
    return (
...
function mergeAsArray<T = AnyFn>(to: T[] | T | undefined, from: T | T[]) {
