...
import { isSameVNodeType } from '../type-node/type-node.util';
import {
  Hook,
  TransitionElement,
...
export function getLeavingNodesForType(
  ...
  let leavingVNodesCache = leavingVNodes.get(vnode.props.nodeName)!;
  if (!leavingVNodesCache) {
    leavingVNodesCache = Object.create(null);
    leavingVNodes.set(vnode.props.nodeName, leavingVNodesCache);
  }
...
export function getTransitionRawChildren(
  ...
  for (let i = 0; i < children.length; i++) {
    ...
    // else if (keepComment || child.props.nodeName !== 'comment') {
