...
export function initParams<Props extends TypeProps = TypeProps, A extends Attributes = Attributes>(element: TypeNode<Props, A>, params = {} as Props): Props {
  ...
  //   // 如果已经配置了默认值，则使用默认值
  //   // (params as any)[key] ??= (this.props as any)?.[key];
  // }
  assignProps(element, params);
  return element.props as unknown as Props;
}
