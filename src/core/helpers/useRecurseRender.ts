...
import { processIfFalse, processIfTrue } from '../transforms/vIf';
 ...
 * @param container
 */
export function useRecurseRender(element: TypeElement, container: RendererElement) {
  // 渲染当前元素。
  element.render();
  ...
  element.childNodes.forEach(child => {
    ...
    if (child instanceof TypeElement) {
      ...
      useRecurseRender(child, container);
      if (Object.hasOwnProperty.call(child.props, 'vIf')) {
        if (toRaw(child.props.vIf)) {
          processIfTrue(child, container);
        } else {
          processIfFalse(child, container);
        }
      }
    } else {
      child.render();
