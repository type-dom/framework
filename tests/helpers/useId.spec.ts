...
//   logger.warn('serverResult app is ', app)
...
//   },
// })
class BasicComponentWithUseId extends TypeFragment {
  className = 'BasicComponentWithUseId';
  override setup() {
    const id1 = useId()
    const id2 = useId()
    transformSlot(this, [id1, ' ', id2]);
  }
}
...
describe('useId', () => {
  test('basic', async () => {
    expect(
      await getOutput(() => {
        const app = createApp(new BasicComponentWithUseId())
        return [app, []]
      }),
    ).toBe('v-0 v-1')
  })
  ...
  test('async component', async () => {
    // const factory = (delay1: number, delay2: number) => {
    //   const p1 = promiseWithDelay(new BasicComponentWithUseId(), delay1);
    //   const p2 = promiseWithDelay(new BasicComponentWithUseId(), delay2);
    ...
    //         ele?.addChildren(id1, ' ', id2, ' ', AsyncOne, ' ');
    //       },
    //     })
    //   );
    //   return [app, [p1, p2]];
  ...
  //     const p1 = promiseWithDelay(null, delay1)
  //     const p2 = promiseWithDelay(new BasicComponentWithUseId(), delay2)
  //     const AsyncInner = p2; // defineAsyncComponent(() => p2)
  //
  ...
  //     //   },
  //     // })
  //     class AsyncSetup extends TypeFragment {
  //       className = 'AsyncSetup';
  //       override async setup() {
  //         await p1;
  //         this.addChild(await AsyncInner())
  //       }
  //     }
  //
