...
function mockProps(extra: TransitionProps = {}, withKeepAlive = false) {
  const cbs: {
  ...
  const props: TransitionProps = {
    ...
    onEnter: vi.fn((el, done) => {
      cbs.doneEnter[el?.innerHTML] = done
    }),
    onAfterEnter: vi.fn(),
    onEnterCancelled: vi.fn(),
    onBeforeLeave: vi.fn(),
    onLeave: vi.fn((el, done) => {
      cbs.doneLeave[el?.innerHTML] = done
    }),
    onAfterLeave: vi.fn(),
...
//     expect(props[key as keyof TransitionProps]).toHaveBeenCalledTimes(
//       calls[key],
//     )
//   })
// }
...
//   return tester({
//     trueBranch: () => new Div(), // h('div'),
//     falseBranch: () => new Span(), // h('span'),
...
//   type CompProps = { msg: string } & DivProps;
//   class CompA extends TypeDiv {
//     className = 'CompA';
//     constructor(params: CompProps) {
//       super(params);
//       this.addChild(params.msg);
//     }
//   }
//   // test HOC
//   // const CompB = ({ msg }: { msg: string }) => h(CompC, { msg })
//   type CompBProps = { msg: string } & FragmentProps;
//   class CompB extends TypeFragment<CompBProps> {
//     className = 'CompB';
//     constructor(params: CompBProps) {
//       super(params);
//       this.addChild(new CompC({ msg: params.msg }));
//     }
//   }
//   // const CompC = ({ msg }: { msg: string }) => h('span', msg)
//   type CompCProps = { msg: string } & SpanProps;
//   class CompC extends TypeSpan<CompCProps> {
//     className = 'CompC';
//     constructor(params: CompCProps) {
//       super(params);
//       this.addChild(params.msg);
//     }
//   }
//   return tester({
//     trueBranch: () => new CompA({ msg: 'foo'}), // h(CompA, { msg: 'foo' }),
//     falseBranch: () => new CompB({ msg: 'bar'}), // h(CompB, { msg: 'bar' }),
...
//   //     return () => new Div({ slot: count}) //  h('div', count.value)
//   //   },
//   // }
//   const count = signal(0);
//   const trueComp = new Div({
//     slot: count
//   })
//   // const falseComp = {
//   //   setup() {
...
//   //   },
//   // }
//   const falseComp = new Span({
//     slot: count,
//   })
//   return tester(
//     {
//       trueBranch: () => trueComp, // h(trueComp),
//       falseBranch: () => falseComp, // h(falseComp),
...
describe('BaseTransition', () => {
  ...
  //   //   const { root } = mount(
  //   //     props,
  //   //     () => (toggle.get() ? trueBranch() : falseBranch()),
  //   //     // withKeepAlive,
  //   //   )
  //   //
  //   //   // start toggle
  //   //   toggle.set(false)
  //   //   await nextTick()
  //   //   expect(root.innerHTML).toBe(`${trueSerialized}${falseSerialized}`)
  //   //   // leave should be triggered
  //   //   expect(props.onBeforeLeave).toHaveBeenCalledTimes(1)
  //   //   assertCalledWithEl(props.onBeforeLeave, trueSerialized)
  //   //   expect(props.onLeave).toHaveBeenCalledTimes(1)
  ...
  //   //   expect(root.innerHTML).toBe(`${falseSerialized}${trueSerialized}`)
  //   //   if (!withKeepAlive) {
  //   //     expect(props.onAfterLeave).toHaveBeenCalledTimes(1)
  //   //     assertCalledWithEl(props.onAfterLeave, trueSerialized)
  //   //   } else {
  ...
  test('should not error on KeepAlive w/ function children', () => {
    expect(() => mount({}, () => () => new Div(), /*true*/)).not.toThrow()
  })
