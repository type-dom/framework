...
// declare var __TD_HMR_RUNTIME__: HMRRuntime
...
describe('defineCustomElement', () => {
  ...
  describe('props', () => {
    ...
    type Props = {
      foo?: string | null;
      bar?: { x: string };
      bazQux?: ISlotItem<IChild>;
      value?: object;
    } & FragmentProps;
    class E extends TypeHtml<Props> {
      className = 'E';
      dom: HTMLDivElement;
      ...
      override setup() {
        this.addChildren(
          ...
          new Div({
            slot: this.props.bazQux || (this.props.bar && this.props.bar.x),
          })
        );
      }
    }
    ...
    test('props via properties', async () => {
      ...
      // e.props.foo = 'one'
      // e.props.bar = { x: 'two' }
      // container.appendChild(e.dom)
      e.mount(container);
      // expect(e.dom.shadowRoot!.innerHTML).toBe('<div>one</div><div>two</div>')
      expect(e.dom.innerHTML).toBe('<div>one</div><div>two</div>');
      ...
      // e.props.foo = 'three' // TypeDom 中 普通字符串的值，不是响应式的。
      // await nextTick()
      // // expect(e.dom.shadowRoot!.innerHTML).toBe('<div>three</div><div>two</div>')
      // expect(e.dom.innerHTML).toBe(
      //   '<div>three</div><div>two</div>'
      // );
      // expect(e.dom.getAttribute('foo')).toBe('three')
      //
      // e.props.foo = null
      // await nextTick()
      // // expect(e.dom.shadowRoot!.innerHTML).toBe('<div></div><div>two</div>')
      // expect(e.dom.innerHTML).toBe('<div></div><div>two</div>');
      // expect(e.dom.hasAttribute('foo')).toBe(false)
      //
      // e.props.foo = undefined
      // await nextTick()
      // // expect(e.dom.shadowRoot!.innerHTML).toBe('<div></div><div>two</div>')
      // expect(e.dom.innerHTML).toBe('<div></div><div>two</div>');
      // expect(e.dom.hasAttribute('foo')).toBe(false)
      // expect(e.props.foo).toBe(undefined)
      //
      // e.props.bazQux = 'four'
      // await nextTick()
      // // expect(e.dom.shadowRoot!.innerHTML).toBe('<div></div><div>four</div>')
      // expect(e.dom.innerHTML).toBe('<div></div><div>four</div>');
      // expect(e.dom.getAttribute('baz-qux')).toBe('four')
    })
