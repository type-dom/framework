...
// import { onWatcherCleanup } from '../../src/reactivity/watch';
...
describe('watch', () => {
  ...
  test('nested batch edge case', () => {
    ...
    function useClamp(src: Ref<number>, min: number, max: number) {
      return computed(
        () => {
          return (src.set(clamp(src.get(), min, max)))
        },
       (val)=> {
          src.set(clamp(val, min, max))
        },
      )
    }
  ...
  test('should ensure correct execution order in batch processing', () => {
    ...
    const n1 = signal(0)
    const n2 = signal(0)
    const sum = computed(() => n1.get() + n2.get())
    watch(n1, () => {
      dummy.push(1)
      n2.set(n2.get() + 1)
    })
