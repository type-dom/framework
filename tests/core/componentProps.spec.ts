...
import { Div, TypeFragment, FragmentProps, defaultProps } from '../../src';
...
describe('component props', () => {
  ...
  test('default value', () => {
    ...
    //   },
    // }
    interface CompProps extends FragmentProps {
      foo?: number,
      bar?:  Mock<() => { a: number }> | { b: number },
      baz?:  Mock<() => { b: number }>
    }
    class Comp extends TypeFragment<CompProps> {
      className = 'Comp';
      constructor(params: CompProps = {}) {
        ...
        super(defaultProps(params, defaults));
        this.className = 'Comp';
    ...
    expect(proxy.props.foo).toBe(2)
    const prevBar = proxy.props.bar
    // expect(proxy.props.bar).toEqual({ a: 1 })
    expect(proxy.props.baz).toEqual(defaultBaz)
    ...
    expect(proxy.props.foo).toBe(3)
    expect(proxy.props.bar).toEqual({ a: 1 })
    expect(proxy.props.bar).toBe(prevBar)
    expect(defaultFn).toHaveBeenCalledTimes(1)
    ...
    expect(proxy.props.foo).toBe(1)
    expect(proxy.props.bar).toEqual({ b: 2 })
    expect(defaultFn).toHaveBeenCalledTimes(1)
    ...
    expect(proxy.props.foo).toBe(3)
    expect(proxy.props.bar).toEqual({ b: 3 })
    expect(defaultFn).toHaveBeenCalledTimes(1)
    ...
    expect(proxy.props.foo).toBe(1)
    expect(proxy.props.bar).toEqual({ b: 4 })
    expect(defaultFn).toHaveBeenCalledTimes(1)
  })
