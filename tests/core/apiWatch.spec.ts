...
describe('api: watch', () => {
  it('effect', async () => {
    const state = toSignals({ count: 0 })
    let dummy
    effect(() => {
      dummy = state.count.get()
    })
    expect(dummy).toBe(0)
  ...
  it('watching single source: signal', async () => {
  ...
  it('watching single source: array', async () => {
    const array = signal([] as number[])
    const spy = vi.fn()
    watch(array, spy, { deep: true })
    array.get().push(1)
    trigger(array)
    await nextTick()
  ...
  //   const spy1 = vi.fn()
  //   const array = [spy1] // reactive([spy1])
  //   const spy2 = vi.fn()
  //   watch(array, spy2, { immediate: true })
  //   // expect(spy1).toBeCalledTimes(0) // error 1
  //   expect(spy1).toBeCalledTimes(1)
  //   // expect(spy2).toBeCalledWith([spy1], undefined, expect.anything())
  //   expect(spy2).toBeCalledWith([spy1], [undefined])
  // })

  it('should not unwrap signals in a reactive source array', async () => {
    const val = signal({ foo: 1 })
    const array = signal([val])
    const spy = vi.fn()
    watch(array, spy, { immediate: true })
    expect(spy).toBeCalledTimes(1)
    // expect(spy).toBeCalledWith([val.get()], undefined, expect.anything())
    expect(spy).toBeCalledWith([val.get()], undefined)
    // deep by default
    val.get().foo++
    trigger(val)
    await nextTick()
    ...
    expect(spy).toBeCalledWith([val.get()], [val.get()])
  })
  ...
  it('watching single source: computed signal', async () => {
    const count = signal(0)
    const plus = computed(() => count.get() + 1)
    let dummy
  ...
  it('directly watching reactive object (with automatic deep: true)', async () => {
    ...
    trigger(src)
    await nextTick()
    expect(dummy).toBe(1) // error undefined
  })
  ...
  it('directly watching reactive array with explicit deep: false', async () => {
    const val = signal(1)
    const array: Signal<(Signal<number> | number)[]> = signal([val]) //  reactive([val])
    const spy = vi.fn()
    ...
    // val.set(val.get() + 1);
    // await nextTick();
    // expect(spy).toBeCalledTimes(1); // error 2
    ...
    expect(spy).toBeCalledWith([val.get(), 2], [val.get()]);
  })
  ...
  it('watching multiple sources', async () => {
    ...
    let dummy
    watch([() => state.get().count, count, plus], (vals, oldVals) => {
      dummy = [vals, oldVals]
      // assert types
      // vals.concat(1)
      // oldVals.concat(1)
  ...
  it('watching multiple sources: undefined initial values and immediate: true', async () => {
    ...
    await nextTick()
    expect(called).toBe(true) // error false
  })

  it('watching multiple sources: readonly array', async () => {
    const state = signal({ count: 1 })
    const status = signal<boolean>(false)

    let dummy
    watch(() => [state.get().count, status.get()] as [number, boolean], (vals, oldVals) => {
      dummy = [vals, oldVals]
      // const [count] = vals
  ...
  //   const cleanup = vi.fn()
  //   let dummy
  //   const stop = watch(count, (count: number, prevCount: number, onCleanup) => {
  //     onCleanup(cleanup)
  //     dummy = count
  //   })
  //
  ...
  //     override setup() {
  //       effect(() => {
  //         assertion(count.get(), count2.get())
  //       })
  //       transformSlot(this, count)
  //       // return () => count.get()
  //     }
  //   }
  //   const root = new Div({
  ...
  //   })
  //   // render(h(Comp), root)
  //   expect(assertion).toHaveBeenCalledTimes(1) // error 0
  ...
  it('flush: pre triggered in component setup should be buffered and called before mounted', () => {
    ...
    // expect(calls).toMatchObject(['watch 3', 'mounted']) // error  ["watch 1", "watch 2", "watch 3", "mounted",]
    expect(calls).toMatchObject(["watch 1", "watch 2", 'watch 3', 'mounted'])
  })
  ...
  it('flush: post watcher should fire after template refs updated', async () => {
    ...
    class App extends TypeFragment {
      className = 'App';
      override setup() {
        const domRef = signal<HTMLElement | undefined>(undefined)
        ...
        const p = new P({ refDom: domRef })
        // p 在默认加载时是不存在的， refDom 无法绑定domRef；
        transformSlot(this, () => toggle.get() ? p : undefined)
        // return () => {
    ...
    await nextTick()
    console.warn('dom is ', dom)
    expect((dom as any)?.tagName).toBe('P')
  })
  ...
  it('deep', async () => {
    ...
    state.get().nested.count.set(state.get().nested.count.get() + 1)
    trigger(state)
    await nextTick()
    ...
    // nested map mutation
    state.get().map.set('a', 2)
    trigger(state)
    await nextTick()
    expect(dummy).toEqual([1, 2, 2, true])

    // nested set mutation
    state.get().set.delete(1)
    trigger(state)
    await nextTick()
    expect(dummy).toEqual([1, 2, 2, false])
  })

  it('watching deep signal', async () => {
    const count = signal(0)
    const double = computed(() => count.get() * 2)
    const state = signal([count, double])

    let dummy
    watch(
      () => state.get(),
      state => {
        dummy = [state[0].get(), state[1].get()]
      },
      { deep: true },
    )
  ...
  it('deep with symbols', async () => {
    ...
    watch(() => state.get(), spy, { deep: true })

    expect(spy).toHaveBeenCalledTimes(0) // error 1

    state.get()[symbol1][symbol2] = 2
    trigger(state)
    await nextTick()
  ...
  it('immediate: triggers when initial value is null', async () => {
    ...
    const spy = vi.fn()
    watch(() => state.get(), spy, { immediate: true })
  ...
  test('should force trigger on trigger with toRef from reactive', async () => {
    const foo = signal({ bar: 1 })
    const bar = toSignal(foo.get(), 'bar')
    const spy = vi.fn()
  ...
  // #2125
  test('effect should not recursively trigger itself', async () => {
    const spy = vi.fn()
    const price = signal(10)
    const history = signal<number[]>([])
    effect(() => {
      history.get().push(price.get())
      spy()
    })
    await nextTick()
    expect(spy).toHaveBeenCalledTimes(1)
  ...
  test('computed refs should not trigger watch if value has no change', async () => {
    ...
    const price = computed(() => source.get() === 0)
    watch(price, spy)
  ...
  //   const Comp = defineComponent({
  //     setup() {
  //       const comp = signal<ComponentPublicInstance | undefined>()
  ...
  //     spy()
  //   })
  //   foo.set(foo.get().slice())
  //   await nextTick()
  //   // expect(spy).toBeCalledTimes(1) // todo 新旧值都是[1]，为什么要监听？？？
  // })

  it('watching multiple sources: computed', async () => {
    let count = 0
    const value = signal('1')
    const plus = computed(() => !!value.get())
    watch([plus], () => {
      count++
    })
    value.set('2')
    await nextTick()
    expect(plus.get()).toBe(true)
    expect(count).toBe(0) // error 1
  })
  ...
  // test('effect should keep running if created in a detached scope', async () => {
  ...
  test.each(options)('$name', async option => {
    ...
    expect(cb).toHaveBeenCalledTimes(1) // error 2
  ...
  it('watching reactive with deep: false', async () => {
    ...
    trigger(state)
    await nextTick()
    expect(spy).toHaveBeenCalledTimes(1) // error 0
  })
  ...
  test('effect should not fire on computed deps that did not change', async () => {
    const a = signal(0)
