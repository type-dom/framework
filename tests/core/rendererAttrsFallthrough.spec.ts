...
import {
  ...
  DivProps,
  TypeDiv,
  TypeFragment, FragmentProps, MaybeRef, HTMLAttributes
} from '../../src';
...
describe('attribute fallthrough', () => {
  it('should allow attrs to fallthrough', async () => {
    ...
    //   },
    // }
    class Hello extends TypeFragment {
      className = 'Hello';
      override setup() {
        const count = signal(0)
        ...
        this.addChild(new Child({
          foo: computed(() => count.get() + 1),
          class: computed(() => 'c' + count.get()),
          attrObj: {
            id: 'test',
            'data-id': computed(() => count.get() + 1),
          },
          styleObj: computed(() => ({ color: count.get() ? 'red' : 'green' })),
          onClick: inc,
        }))
      }
    }
    ...
    class Child extends TypeDiv<ChildProps> {
      ...
      constructor(params: ChildProps) {
        ...
        // this.addAttrObj({ class: 'c2' });
        // this.addStyleObj({ fontWeight: 'bold' })
  ...
  it('should only allow whitelisted fallthrough on functional component with optional props', async () => {
    ...
    //   )
    // }
    type ChildProps = { foo?: Ref<number>, id?: string } & DivProps;
    class Child extends TypeDiv<ChildProps> {
      className = 'Child';
      constructor(params: ChildProps) {
        super(params);
  ...
  it('should track this.$attrs access in slots', async () => {
    ...
    //   template: `<slot/>`,
    // }
    class GrandChild extends TypeFragment {
    ...
    interface ChildProps extends DivProps {
      attrObj?: { foo?: MaybeRef<number> } & HTMLAttributes;
    }
    class Child extends TypeDiv<ChildProps> {
      className = 'Child';
      constructor(params: ChildProps) {
        super(params);
        this.addChild(new GrandChild({ slot: params.attrObj?.foo }));
      }
    }
