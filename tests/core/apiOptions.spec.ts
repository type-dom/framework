...
describe('api: options', () => {
  ...
  test('provide/inject refs', async () => {
    ...
    app.mount(root);
    expect(serializeInner(app)).toBe(`<fragment>1</fragment>`); //  "<fragment>1</fragment>"

    n.set(n.get() + 1);
    await nextTick();
    expect(serializeInner(app)).toBe(`<fragment>3</fragment>`);
  })
  ...
  describe('options merge strategies', () => {
    ...
    test('this.$options.inject', () => {
      ...
      onCreated(() => {
        expect(app.$options.inject?.a).toEqual('a')
        expect(app.$options.inject?.b).toEqual('b')
        expect(app.$options.inject?.c).toEqual({ from: 'd' })
        expect(app.provides?.a).toBe(1)
        expect(app.provides?.b).toBe(2)
        expect(app.provides?.c).toBe(3)
      }, app);
