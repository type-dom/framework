...
import {
  ...
  triggerEvent,
  Div,
  TypeFragment, FragmentProps, onMounted, TypeHead, HeadProps
} from '../../src';
// import { onCreated } from '../../src/core/apiLifecycle';
// import {
//   type TestElement,
//   computed,
//   createApp,
...
describe('api: options', () => {
  test('data', async () => {
    ...
    root.mount(document.body);  // 否则无法触发 comp onClick;
    // render(h(Comp), root)
    // render(comp, root);

    expect(root.dom.innerHTML).toBe(`<div>1</div>`)

    triggerEvent(root.children[0], 'click')
    await nextTick()
    expect(root.dom.innerHTML).toBe(`<div>2</div>`)
  })
  ...
  test('provide/inject refs', async () => {
    ...
    class Child extends TypeFragment {
      className = 'Child';
      override setup() {
        const n = inject<Signal<number>>('n')!;
        const np = inject<Computed<number>>('np')!;
        this.addChild(computed(() => n.get() + np?.get()));
      }
    }
    const app = createApp(new Parent())
  ...
  test('extends', () => {
    ...
    //   },
    // })
    abstract class Base extends TypeFragment {
      // className = 'Base'
      a: number;
      b: number;
    ...
    expect(renderToString(new Comp())).toBe(`12`);
    expect(calls).toEqual(['base', 'comp']) // error comp not found
  })
  ...
  //     render() {
  //       return `${this.props.msg1},${this.props.msg2}`
  //     },
  //   })
  //
  ...
  //     override setup() {
  //       provide('a', 'root');
  //       this.addChildren(new Mid(), new MidWithProvide())
  //     }
  //   }
  //
  //   // const Mid = {
  ...
  //   //   },
  //   // } as any
  //   class Mid extends TypeFragment {
  //     className = 'Mid';
  //     override setup() {
  //       this.addChild(new Child())
  //     }
  ...
  //   //   },
  //   // } as any
  //   class MidWithProvide extends TypeFragment {
  //     className = 'MidWithProvide';
  //     override setup() {
  //       provide('a', 'midWithProvide');
  //       this.addChild(new Child())
  //     }
  //   }
  //
  ...
  //     className = 'Child';
  //     override setup() {
  //       const a = inject('a') as string;
  //       this.addChild(a);
  //     }
  //   }
  //   const root = new Root();
  //   root.mount()
  //   expect(renderToString(root)).toBe(
  //     'root midWithProvide', // 'root midWithProvide midWithMixinProvide',
  //   )
  // })
  ...
  describe('options merge strategies', () => {
    ...
    //     },
    //     created() {
    //       expect(this.props.data).toBeInstanceOf(Function)
    //       expect(this.props.data()).toEqual({
    //         foo: 3,
    //         bar: 2,
    //         baz: 4,
    //       })
    ...
    test('this.props.inject', () => {
      ...
      //   inject: { b: 'b', c: { from: 'd' } },
      //   created() {
      //     expect(this.props.inject.a).toEqual('a')
      //     expect(this.props.inject.b).toEqual('b')
      //     expect(this.props.inject.c).toEqual({ from: 'd' })
      //     expect(this.a).toBe(1)
      //     expect(this.b).toBe(2)
      ...
      const app = createApp(new Div({
        inject: { a: 'a', b: 'b', c: { from: 'd' } },
        created(this: Div) {
          expect(this.props.inject?.a).toEqual('a')
          expect(this.props.inject?.b).toEqual('b')
          expect(this.props.inject?.c).toEqual({ from: 'd' })
          expect(this.provides?.a).toBe(1)
          expect(this.provides?.b).toBe(2)
          expect(this.provides?.c).toBe(3)
        }
      }));
    ...
    test('this.props.provide', () => {
      ...
      createApp(new Div({
        ...
        created(this: Div) {
          expect(this.props.provide).toBeInstanceOf(Function)
          expect(this.props.provide?.()).toEqual({ a: 1, b: 2 })
        },
        // render: () => null,
      })).mount(document.createElement('div'))
    })

    // test('this.props[lifecycle-name]', () => {
    //   const mixin = {
    //     mounted() {},
    //     beforeUnmount() {},
    //     unmounted() {},
    ...
    //     unmounted() {},
    //     created() {
    //       expect(this.props.mounted).toBeInstanceOf(Array)
    //       expect(this.props.mounted.length).toBe(2)
    //       expect(this.props.beforeUnmount).toBeInstanceOf(Array)
    //       expect(this.props.beforeUnmount.length).toBe(2)
    //       expect(this.props.unmounted).toBeInstanceOf(Array)
    //       expect(this.props.unmounted.length).toBe(2)
    //     },
    //     render: () => null,
    //   }).mount(nodeOps.createElement('div'))
    // })

    // test('this.props[asset-name]', () => {
    //   const mixin = {
    //     components: {
    //       a: {},
    //     },
    ...
    //     },
    //     created() {
    //       expect('a' in this.props.components).toBe(true)
    //       expect('b' in this.props.components).toBe(true)
    //       expect('d1' in this.props.directives).toBe(true)
    //       expect('d2' in this.props.directives).toBe(true)
    //     },
    //     render: () => null,
    //   }).mount(nodeOps.createElement('div'))
    // })

    // test('this.props.methods', () => {
    //   const mixin = {
    //     methods: {
    //       fn1() {},
    //     },
    ...
    //     },
    //     created() {
    //       expect(this.props.methods.fn1).toBeInstanceOf(Function)
    //       expect(this.props.methods.fn2).toBeInstanceOf(Function)
    //     },
    //     render: () => null,
    //   }).mount(nodeOps.createElement('div'))
    // })

    // test('this.props.computed', () => {
    //   const mixin = {
    //     computed: {
    //       c1() {},
    //     },
    ...
    //     },
    //     created() {
    //       expect(this.props.computed.c1).toBeInstanceOf(Function)
    //       expect(this.props.computed.c2).toBeInstanceOf(Function)
    //     },
    //     render: () => null,
    //   }).mount(nodeOps.createElement('div'))
    // })
    ...
    //         this.props.computed = {}
    //       }
    //       this.props.computed.value = () => count.value
    //     },
    //   }
    //   const root = nodeOps.createElement('div')
    //   createApp({
